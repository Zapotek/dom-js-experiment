...
EOHTML
    ...
    def test_anchors_count
        ...
        assert_equal 3, @window.eval( 'document.anchors.length' )
    end

    def test_first_anchors_inner_html
        txt = 'HTML Tutorial'
        assert_equal txt, @document.anchors[0].innerHTML
        assert_equal txt, @window.eval( 'document.anchors[0].innerHTML' )
    end

    def test_get_href
        href = '/js/'
        assert_equal href, @document.getElementById( 'w3s' ).href
        assert_equal href, @window.eval( "document.getElementById( 'w3s' ).href" )
    end
end
